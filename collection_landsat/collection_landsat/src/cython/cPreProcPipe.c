...
void cAdjustLevels (uint8_t* inArr, int n, int m) {
	int i, j, k;
	uint8_t work, min, max;
	...
	for (i = 0; i < n; ++i) {
		for (j = 0; j < m; ++j) {
			if (inArr[indexer(i, j, 0, n, m)] != 0)
				inArr[indexer(i, j, 0, n, m)] = (uint16_t) (((double) (inArr[indexer(i, j, 0, n, m)] - min) / (double) (max - min)) * 255.0);
			if (inArr[indexer(i, j, 1, n, m)] != 0)
				inArr[indexer(i, j, 1, n, m)] = (uint16_t) (((double) (inArr[indexer(i, j, 1, n, m)] - min) / (double) (max - min)) * 255.0);
			if (inArr[indexer(i, j, 2, n, m)] != 0)
				inArr[indexer(i, j, 2, n, m)] = (uint16_t) (((double) (inArr[indexer(i, j, 2, n, m)] - min) / (double) (max - min)) * 255.0);
		}
	}
}
...
void cDownsize (uint16_t* inArr, uint16_t* outArr, int n, int m) {
	int i, j;
